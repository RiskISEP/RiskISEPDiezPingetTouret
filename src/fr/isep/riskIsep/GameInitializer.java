...
import fr.isep.riskIsep.Mission.*;
	...
	private void initPlayers(List<Player> players) {
		...
		switch (players.size()) {
			case 2:
				...
				missions.add(new Mission5());
				missions.add(new Mission8());
				break;
		...
		for (int i = 0; i < players.size(); i++) {
			...
			players.get(i).setMission(missions.get(random.nextInt(missions.size())));
			System.out.println(players.get(i).getName() + " a la mission" + players.get(i).getMission().getName() + " : ");
			System.out.println(players.get(i).getMission().getDescription());
